...
def test_valid_foreign_keys_ciod_module(ciod_module_relationship, ciods, modules):
    for id_path, relationship in ciod_module_relationship.items():
        assert relationship['ciod'] in ciods
        assert relationship['module'] in modules

@pytest.mark.endtoend
def test_valid_foreign_keys_module_attribute(module_attribute_relationship, modules, attributes):
    for id_path, relationship in module_attribute_relationship.items():
        assert relationship['module'] in modules
        assert relationship['tag'] in attributes
...
def test_trace_from_attribute_to_ciod(ciods, ciod_module_relationship,
    ...
    module_attr = {
        "patient-study:00081084:00080121": {
            "type":"3",
            "order":8,
        ...
        "patient-study:00101021:00080121": {
            "type":"3",
            "order":43,
            "module":"patient-study",
            "tag":"(0008,0121)",
    ...
    assert module_attr["patient-study:00081084:00080121"] == module_attribute_relationship["patient-study:00081084:00080121"]
    assert module_attr["patient-study:00101021:00080121"] == module_attribute_relationship["patient-study:00101021:00080121"]
    assert module['patient-study'] == modules['patient-study']
    assert ciod_module["cr-image:patient-study"] == ciod_module_relationship["cr-image:patient-study"]
...
def test_number_of_attribute_appearances(module_attribute_relationship, attributes):
    ...
    assert module_attr[0] == module_attribute_relationship['patient-demographic:00100213']
    assert module_attr[1] == module_attribute_relationship['patient:00100213']
    all_attribute_appearances = [rel for id_path, rel in module_attribute_relationship.items()
...
def test_number_of_module_appearances(ciods, ciod_module_relationship, modules):
    ...
    assert ciod_module == ciod_module_relationship['planar-mpr-volumetric-presentation-state:volume-cropping']
    all_module_appearances = [rel for id_path, rel in ciod_module_relationship.items()
                              if rel == ciod_module]
