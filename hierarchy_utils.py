...
def record_hierarchy_for_module(table: dict) -> dict:
    last_id = [table['id']]
    current_level = -1
...
def update_hierarchy_position(attr: dict, last_id: List[str], current_level: int) -> Tuple[List[str], int]:
...
def format_attribute_fields(attr: dict, last_id: List[str]) -> None:
